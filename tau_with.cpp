...
/*		if(coin_flip(p_sel) && pop.get_time() > t_select){
		...
		if(clicker == 0 && pop.get_time() > t_select) {
			pop.repop_wf_sel(*fitness, fit_parms, p_mut, 1);
...
int main(){
	...
	double stoch_parms[4] = {0.1, 100., 1., 67.};
	...
	double prob_mut_diff[4] = {0., 0., 0., 0.1};
	...
	for(int i=0; i < n_reps; i++){
		...
		runsim_plasticity(mufasa, simba, transf, &fit, fit_p, 2, 1, 1, stoch_parms, sel_parms, n_parms, n_pop, &mutation, mutation_parms, 1./67., left_bound, right_bound, prob_selection, prob_mut_diff, select_flip, false, true, true, true, 10000, 0.5, 5. + 0.5, 0.0001, 0.02);
...
//		delete mutation_parms[i];
//	}
	delete[] mutation_parms;
